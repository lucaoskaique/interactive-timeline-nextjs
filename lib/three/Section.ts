...
export default class Section extends THREE.Group {
    ...
    createIntroSection() {
        let sansTextGeom = new TextGeometry("YEAR IN REVIEW", {
            font: this.timeline.assets.fonts["Helvetiker"],
            size: 60,
            depth: 0,
            curveSegments: 4,
        }).center();
    ...
    addWhooshButton() {
        ...
        let whooshTexture = new THREE.TextureLoader().load(
            "/images/whoooosh.png",
        );
        ...
        texture.anisotropy =
            this.timeline.renderer.capabilities.getMaxAnisotropy();
        texture.magFilter = texture.minFilter = THREE.LinearFilter;
        let material = new THREE.MeshBasicMaterial({
            map: texture,
            transparent: true,
    ...
    createContactSection() {
        ...
        let lineOneGeom = new TextGeometry(
        ...
        let lineTwoGeom = new TextGeometry(
